...
export const addOrderRepo = async (userId: number, values: ordersType) => {
  const { productId, count } = values;
  try {
    const queryText = `
        INSERT INTO orders (user_id, product_id, order_count)
        VALUES ($/user_id/, $/productId/, $/count/)
        ON CONFLICT (user_id, product_id)
        DO UPDATE SET order_count = orders.order_count + EXCLUDED.order_count,updated_at = CURRENT_TIMESTAMP;  
        `;
    await query(queryText, [userId, productId, count]);
...
export const getOrderByUserIdRepo = async (userId: string) => {
  try {
    const result =
      await query(`SELECT p.*,o.status,o.order_count FROM products p 
...
export const changeOrderStatusRepo = async (
  status: string,
  product_id: number,
  user_id: number
) => {
